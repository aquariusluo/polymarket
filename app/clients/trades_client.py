...
from app.clients.base_client import BasePolymarketClient, PolymarketUpstreamError
...
class TradesClient(BasePolymarketClient):
    ...
    def _parse_trade(self, row: dict[str, Any], leader_name: str | None) -> LeaderTrade:
        ...
        if not tx_hash:
            raise PolymarketUpstreamError(f"Missing transaction hash in trade row: {row}")

        wallet = row.get("user") or row.get("wallet") or row.get("proxyWallet") or row.get("owner")
        if not wallet:
            raise PolymarketUpstreamError(f"Missing wallet in trade row: {row}")

        timestamp = row.get("timestamp") or row.get("createdAt") or row.get("time")
        if not timestamp:
            raise PolymarketUpstreamError(f"Missing timestamp in trade row: {row}")

        asset_id = row.get("assetId") or row.get("asset_id") or row.get("asset")
        if asset_id is None:
            raise PolymarketUpstreamError(f"Missing asset_id in trade row: {row}")
    ...
    def fetch_recent_trades(
        ...
        if not isinstance(payload, list):
            raise PolymarketUpstreamError(f"Unexpected trades payload type: {type(payload)}")

        trades: list[LeaderTrade] = []
        for row in payload:
            if not isinstance(row, dict):
                raise PolymarketUpstreamError(f"Unexpected trade row type: {type(row)}")
